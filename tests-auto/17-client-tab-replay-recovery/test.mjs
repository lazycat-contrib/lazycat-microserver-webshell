  ...
  try {
    ...
    if (originalScrollback < 6000) {
      await page.evaluate(async () => {
        const r = await fetch("./api/settings", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ terminal_scrollback: 6000 }) });
        if (!r.ok) throw Error(`settings PATCH HTTP ${r.status}`);
      });
    ...
    if (changedSettings) await page.evaluate(async (value) => {
      const r = await fetch("./api/settings", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ terminal_scrollback: value }) });
