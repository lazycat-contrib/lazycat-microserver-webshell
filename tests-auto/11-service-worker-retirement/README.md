...
- 相关模块和源码入口：`runtime/static/app/bootstrap/legacy_service_worker_retirement.js`、`runtime/static/index.html`、`tests-auto/run-playwright.mjs`、`tests-auto/test-all.sh`
...
- dry-run 中本场景显示 `static=<empty>`，其余当前前端场景映射指定构建目录。
...
也可由 `HEADLESS=1 ./tests-auto/test-all.sh` 自动应用本场景 profile。认证信息只通过 `tests-auto/.env` 或运行环境注入。
